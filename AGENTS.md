...
実装計画は `.claude/skills/impl-plan/` の指示に従い、`plan/[機能名]/` に作成する。フォーマット・必須セクション・粒度の基準はその Skill を参照。
...
- 3ステップ以上のタスクは、実装前に `plan/[機能名]/` へ計画を残す。
- **設計判断が含まれるタスクは、ステップ数にかかわらず実装前に計画を残す**。
- 動作を証明できるまでタスクを完了とマークしない。
- コードを読まずに書かない。
- 変更は必要な箇所のみ。影響範囲を最小化する。
- コンテキスト圧迫やタスクに関係のないノイズが多ければセッション切り替えを提案する。
- 不明点があり、ローカル文書やコードから判断できない場合は、簡潔にユーザーへ確認する。

### Plan 必須の判断基準

以下のいずれかに該当する場合は、ステップ数にかかわらず計画作成から開始する：
...
| ユーザーの「はい」が実装承認か設計承認か不明確 | 迷ったら計画を明文化する |
...
開発者は Skills を実験的に試したい意向がある。タスクに有益そうな Skill があれば積極的に作成・利用する。
...
Codex では、ユーザーが明示的にサブエージェント利用・分担・並列作業を依頼した場合のみサブエージェントを起動する。単に「詳しく調べて」「深く確認して」だけでは起動しない。

| エージェント | 用途 |
| --- | --- |
| `explorer` | コードベース探索・ファイル検索・調査タスク全般 |
| `worker` | 所有範囲が明確な実装・修正タスク |
| `default` | 上記に収まらない、明確に分離できる補助タスク |

**委任ルール：**

- 調査タスクは、ユーザーがサブエージェント利用を明示した場合に限り `explorer` へ委任できる。
- 実装タスクは、詳細な Plan がすでに存在し、担当ファイルや責務を明確に分けられる場合に限り `worker` へ委任する。
- サブエージェントに実装を任せる場合は、担当ファイル・責務・「他の変更を巻き戻さない」ことを明示する。
- ユーザーとの対話・方針決定・最終確認・コミット判断はメインエージェントで行う。
