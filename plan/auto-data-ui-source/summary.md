...
| # | フェーズ名 | 状態 | 詳細ファイル |
| --- | --- | --- | --- |
| 1 | Babel transform 基盤 | 完了 | [01-babel-transform.md](01-babel-transform.md) |
| 2 | demo 手書き属性の撤去 | 完了 | [02-remove-manual-attributes.md](02-remove-manual-attributes.md) |
| 3 | 検証とドキュメント同期 | 完了 | [03-verification-and-docs.md](03-verification-and-docs.md) |

## 完了条件

- [x] host app の `.tsx` / `.jsx` intrinsic element に dev 時だけ `data-ui-source` が自動注入される。
- [x] `examples/demo-app/src/components/*` から手書き `data-ui-source` が削除されている。
- [x] mock generator / prompt / demo context が手書き `data-ui-source` を search 対象にしていない。
- [x] overlay ON 時に、手書きなしの Hero / Workflow / Features / Playground などをクリックして source location が表示される（curl で transformed module 上の `data-ui-source` を確認済み。ブラウザでの最終 UX 確認はユーザー側）。
- [x] session start が自動注入された app root 相対 path を受け取り、既存の server path 変換で対象ファイルを開ける。
- [x] app root 外を指す source location は server 側で拒否される（400）。
- [x] 本番 build 相当の transform には注入されない方針が守られている（`apply: "serve"` 維持）。
...
- **e2e / 自動テスト整備**: 入力 `.tsx` に対する transform 出力スナップショット、overlay TSX 非対象、server 側 path 拒否（絶対 / `..` / Windows separator）、plugin 配列順制約の警告（`uiVariants()` が `react()` より後ろにあると行番号ズレが出る）。core 機能なので回帰検知を固定する。
