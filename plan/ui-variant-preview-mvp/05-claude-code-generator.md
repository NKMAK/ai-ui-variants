...
- [ ] `generator: "claude-code"` で SaveButton に「目立たせて」と指示すると、実際に 3 案（search/replace）が返る
- [ ] 返った変更が worktree に反映され `git diff` で patch 化され、Phase 2 の検証（denylist/数/行数）を通る
- [ ] preview で画面が切り替わり、前へ/次へが機能する
- [ ] `claude` CLI 未検出・タイムアウト・パース失敗が握りつぶされず、エラーとして表示される
- [ ] 生成時に Claude がファイルを書換しない（`--allowedTools ""`、対象ファイルの `git status` が generate 単体では変化しない）
- [ ] `generator` 既定が `mock` のままで、mock 経路も従来どおり動く
- [ ] `pnpm tsc --noEmit` が通る
- [ ] 変更がコミットされている
